...
def rf_frequency_to_B(target_frequency, Rbx):
    ...
    target_B = optimize.newton(B_to_difference, 1E-5)
    return target_B
...
def process_linear(number, show=False):
    ...
    print(f'Transparency at Linear RF Frequency {set_.rf_frequency / C.KILO}kHz, Main Current {set_.main_coil_current}A\n'
...
def process_quadratic(number, show=False):
    ...
    print(f'Transparency at Quadratic RF Frequency {set_.rf_frequency / C.MEGA}MHz, '
...
class ProcessedData:
    ...
    class quadratic:  # only the 1V data
...
def fit_BvsI(show=False):
    ...
    Bfield = np.array([[0, rf_frequency_to_B(L.fRF[i], C.Rb87), rf_frequency_to_B(L.fRF[i], C.Rb85)]
                       for i, I in enumerate(L.Imain) if I == 0]).flatten()
    ...
    Bfield = np.subtract([rf_frequency_to_B(f, C.Rb87) for f in [300E3, 600E3, 1200E3]] +
                         [rf_frequency_to_B(f, C.Rb85) for f in [300E3, 600E3, 800E3]],
...
def fit_fRFvsB(show=False):
    ...
    print(f'g87 = {params87[0] * C.h / C.muB} +- {errors87[0] * C.h / C.muB}\n'
          f'g85 = {params85[0] * C.h / C.muB} +- {errors85[0] * C.h / C.muB}\n'
          f'offset87 (Hz) = {params87[1]} +- {errors87[1]}\n'
          f'offset85 (Hz) = {params85[1]} +- {errors85[1]}')

    if show:
        p = plot.Plot()
        p.line(B87s / C.MICRO, f87s / C.MEGA, format_='ob', markersize=5)
        p.line(B87s / C.MICRO, process.Function.line(B87s, *params87) / C.MEGA, format_='--b')
...
def fit_fRabivsVRF(show=False):
    ...
    if show:
        p = plot.Plot()
        p.line(T.VRF87, np.array(T.omega87) / C.KILO, format_='ob', markersize=5)
        p.line(T.VRF87, process.Function.line(T.VRF87, *params87) / C.KILO, format_='--b')
...
if __name__ == '__main__':
    plot_energies(np.linspace(0, 0.1, 40), C.Rb85)
    plot_rf_freqs(np.linspace(0, 0.1, 40), C.Rb85)
    plot_dynamics(1E1)
